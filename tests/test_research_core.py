...
from research_core.routing import BACKOFF_SEQUENCE, route_mixture
...
class ResearchCoreTests(unittest.TestCase):
    def _request(self):
        ...
        return PracticeRequest(
            categories=raw['categories'], parent_distribution=raw['parent_distribution'],
            evidence=[EvidenceRecord(**x) for x in raw['evidence']],
            permitted_categories=raw['permitted_categories'], kappa_company=raw['kappa_company'],
            temporal_decay_rate_per_day=raw['temporal_decay_rate_per_day'],
            routing_levels=[RoutingLevel(**x) for x in raw['routing_levels']],
            routing_gamma=raw['routing_gamma'], support_tau=raw['support_tau'],
    ...
    def test_eq5_unauthorized_evidence_contributes_zero_mass(self):
        counts,mass=weighted_counts([EvidenceRecord('ownership',False,1.0,0)],['ownership','evidence'],decay_rate_from_half_life(90))
        self.assertEqual(counts['ownership'],0.0); self.assertEqual(mass,0.0)
    ...
    def test_eq4_hierarchical_borrowing_is_exposed(self):
        levels=hierarchical_partial_pooling({'a':0.5,'b':0.5},{'a':1},{'b':1},{'a':2},['a','b'],5,5,5)
    ...
    def test_eq8_9_backoff_sequence_and_routing(self):
        ...
        levels=[RoutingLevel('L0',0,True,True,0.5,{'a':1,'b':0}),RoutingLevel('L5',5,True,True,1.0,{'a':0,'b':1})]
        mix,prov=route_mixture(levels,['a','b'],0.7)
        self.assertAlmostEqual(sum(mix.values()),1.0); self.assertEqual(len(prov),2)
    ...
    def test_demo_is_candidate_side_and_deterministic(self):
        a=prioritize_followups(self._request()); b=prioritize_followups(self._request())
        self.assertEqual(a,b); self.assertEqual(a['mode'],'ordered_practice_priority')
        self.assertIn('not a prediction',a['disclaimer']); self.assertNotIn('collaboration',a['priority_order'])
